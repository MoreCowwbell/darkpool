...
def plot_buy_ratio_series(db_path: Path, output_dir: Path, symbols: list[str], plot_mode: str = "log") -> None:
    ...
    try:
        for symbol in symbols:
            df = conn.execute(
                """
                SELECT date, estimated_bought, estimated_sold, buy_ratio, total_off_exchange_volume, has_finra_data
            ...
            # Check if this is a Polygon-only ticker (no FINRA data)
            is_polygon_only = df["has_finra_data"].notna() & ~df["has_finra_data"]
            has_any_finra = df["has_finra_data"].any() if "has_finra_data" in df.columns else True
            ...
            # Scale marker sizes by volume (use fixed size for Polygon-only tickers with no volume)
            volumes = df["total_off_exchange_volume"].fillna(0).values.astype(float)
            if volumes.max() == 0:
                # All Polygon-only data - use fixed marker size
                marker_sizes = np.full(len(volumes), (MARKER_SIZE_MIN + MARKER_SIZE_MAX) / 2)
            else:
            ...
            # Add Polygon-only indicator if no FINRA data
            if not has_any_finra:
                title_suffix += " [Polygon-only]"
